...
func main() {
	...
	//var c2 *C = &c
	//var c3 interface{}
	//codec.NewDecoderBytes(b, new(codec.CborHandle)).MustDecode(&c2)

	//if err := codec.NewDecoderBytes(b, new(codec.CborHandle)).MustDecode(&c3); err != nil {
	//	log.Error().Err(err).Msg("Failed CBor Decoding")
	//}
	//fmt.Printf(" after: %v\n", c)
	//fmt.Println(c.Amt)
	//fmt.Println(c.Fun)
